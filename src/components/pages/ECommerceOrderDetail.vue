          ...
          <b-table
            ...
            <template v-slot:cell(item)="data">
              <div class="media align-items-center">
                <img :src="`${publicUrl}img/uikit/${data.item.image}`" class="d-block ui-w-40 ui-bordered mr-4" alt>
                <div class="media-body">
                  <a href="javascript:void(0)" class="d-block text-body">{{data.item.title}} <span class="text-muted">#{{data.item.id}}</span></a>
                  <small>
            ...
            <template v-slot:cell(quantity)="data">x{{data.item.quantity}}</template>
            <template v-slot:cell(price)="data">{{formatPrice(data.item.price)}}</template>
